...
myslurm <- tweak(
  batchtools_slurm,
  template = "batchtools.slurm.iid.3d.tmpl",
...
) %dopar%
  {
    ...
    for (i in seq_along(nobs)) {
      for (j in seq_along(models)) {
        # set.seed(b + 2025 * i)
        xs <- simu_rdens(n = nobs[i], model = models[j], d = 3L)
...
raw_output_file <- file.path(path, "raw_ISE_iid_3d.csv")
write.csv(raw_results, raw_output_file, row.names = FALSE)

print("Raw results saved to raw_ISE_iid_3d.csv")
